import { getSupportChatWsToken } from "./api";
import { CHAT_ENABLED } from "../config/chatConfig";

const WS_URL = "wss://skladmarket.uz/api/v1/ws/support";
...
function emit(event, payload) {
  listeners.get(event)?.forEach((handler) => {
    try {
      ...
      console.error(`[supportChatSocket] listener for "${event}" threw`, err);
    }
  });
}

export function onSupportChatEvent(event, handler) {
...
export function subscribeSupportThread(threadId) {
  if (!threadId) return;
...
function trackPendingSend(threadId, clientId) {
  const timeoutId = setTimeout(() => {
    ...
    console.warn(`[supportChatSocket] send_timeout: no echo for client_id=${clientId} thread_id=${threadId} after ${SEND_TIMEOUT_MS}ms`);
    emit("send_timeout", { client_id: clientId, thread_id: threadId });
  }, SEND_TIMEOUT_MS);
  pendingSends.set(clientId, timeoutId);
}

export function confirmSupportChatSend(clientId) {
...
export function sendSupportChatMessage(threadId, body, clientId) {
  ...
  connectSupportChatSocket(currentLang);
  return true;
}

export function sendSupportTyping(threadId) {
  return send({ event: "typing", thread_id: threadId });
}

export function sendSupportRead(threadId, messageIds) {
...
export async function connectSupportChatSocket(lang = "uz") {
  ...
  let wsToken;
  try {
    const data = await getSupportChatWsToken();
  ...
  const ws = new WebSocket(`${WS_URL}?token=${encodeURIComponent(wsToken)}&lang=${encodeURIComponent(currentLang)}`);
  ...
  ws.onclose = (evt) => {
    ...
    console.warn(`[supportChatSocket] closed code=${evt.code} reason="${evt.reason}" wasClean=${evt.wasClean}`);
    emit("close");
    scheduleReconnect();
  };

  ws.onerror = (evt) => {
    console.error("[supportChatSocket] socket error", evt);
    ws.close();
  };
}
