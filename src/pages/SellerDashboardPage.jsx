        ...
        <div style={{ scrollbarWidth: "none" }} className="bg-white dark:bg-[#0D0D0D] rounded-2xl border border-ink-100 dark:border-[#1C1C1C] px-4 py-2 flex gap-3 mb-5 sm:mb-6 overflow-x-auto transition-colors">
          {tabs.map((t) => (
            <button
              ...
              className={`relative sm:py-2.5 text-xs sm:text-sm hover:text-black sm:px-0 px-4 py-3 sm:rounded-0 rounded-xl dark:hover:text-[#FFFFFF] font-medium transition-colors whitespace-nowrap ${activeTab === t.id ? "text-black dark:text-[#FFFFFF] sm:border-none border " : "text-[#8A8A8A]"
                }`}
            >
              {activeTab === t.id && (
